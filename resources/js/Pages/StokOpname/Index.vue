...
const deleteOpname = (id) => {
    if (confirm('Yakin ingin menghapus data stok opname ini? Perubahan stok yang terjadi akibat opname ini akan dibatalkan/dikembalikan ke stok semula.')) {
        router.delete(route('stok-opname.destroy', id), {
            preserveScroll: true
        });
    }
};
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex justify-between items-center">
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">Data Stok Opname (Penyesuaian Stok)</h2>
                <Link :href="route('stok-opname.create')" class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-md text-sm font-medium transition shadow-sm">
                    + Mulai Stok Opname
                </Link>
            </div>
        </template>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-gray-100">
                    <div class="p-6 text-gray-900">
                        <div class="flex justify-between items-center mb-6">
                            <h3 class="text-lg font-medium text-gray-900">Riwayat Stok Opname</h3>
                            <div class="w-full md:w-1/3 relative">
                                <input 
                                    v-model="search" 
                                    type="text" 
                                    placeholder="Cari No. Transaksi / Penanggung Jawab..." 
                            ...
                            <table class="w-full text-sm text-left">
                                <thead class="text-xs text-gray-700 uppercase bg-gray-50 border-b">
                                    <tr>
                                        ...
                                        <th class="px-4 py-3 text-center">Jml Barang Dicek</th>
                                        <th class="px-4 py-3 text-center">Aksi</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr v-for="item in stokOpnames.data" :key="item.id" class="border-b hover:bg-gray-50/50">
                                        ...
                                        <td class="px-4 py-3 font-semibold">{{ item.penanggung_jawab }}</td>
                                        ...
                                        <td class="px-4 py-3 text-center space-x-2 whitespace-nowrap">
                                            <Link :href="route('stok-opname.show', item.id)" class="text-blue-600 hover:text-blue-900 bg-blue-50 px-2 py-1 rounded text-xs font-medium border border-blue-200">
                                                Detail
                                            </Link>
                                            <button @click="deleteOpname(item.id)" class="text-red-600 hover:text-red-900 bg-red-50 px-2 py-1 rounded text-xs font-medium border border-red-200">
                                                Hapus
                                            </button>
                                        </td>
                                    </tr>
                                    <tr v-if="stokOpnames.data.length === 0">
                                        <td colspan="5" class="px-4 py-8 text-center text-gray-500">Belum ada riwayat stok opname.</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                        ...
                        <!-- Pagination -->
                        <div v-if="stokOpnames.links && stokOpnames.links.length > 3" class="mt-6">
                            <div class="flex flex-wrap -mb-1">
                                <template v-for="(link, p) in stokOpnames.links" :key="p">
