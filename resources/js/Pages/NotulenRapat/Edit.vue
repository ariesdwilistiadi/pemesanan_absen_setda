...
const initQuill = async () => {
    ...
    fields.forEach(field => {
        ...
        if (editorContainer && window.Quill) {
            ...
            // Set content dari database
            if (props.notulen[field]) {
                quillInstances[field].root.innerHTML = props.notulen[field];
            }
        }
    });
};
...
const form = useForm({
    absen_rapat_id: props.notulen.absen_rapat_id || '',
    ketua_id: props.notulen.ketua_id || '',
    sekretaris_id: props.notulen.sekretaris_id || '',
    pencacat_id: props.notulen.pencacat_id || '',
    ketua_manual: props.notulen.ketua_manual || '',
    sekretaris_manual: props.notulen.sekretaris_manual || '',
    pencacat_manual: props.notulen.pencacat_manual || '',
    pembukaan: props.notulen.pembukaan || '',
    pembahasan: props.notulen.pembahasan || '',
    peraturan: props.notulen.peraturan || '',
    penutup: props.notulen.penutup || '',
    peserta_mode: props.notulen.peserta_mode || 'terlampir',
});
...
const submit = () => {
    ...
    form.put(route('notulen.update', props.notulen.id), {
        onSuccess: () => {
            alert('Notulen berhasil diupdate!');
        },
        onError: (errors) => {
            console.error(errors);
            alert('Gagal mengupdate: ' + Object.values(errors).join(', '));
        }
    });
};
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center gap-4">
                ...
                <h2 class="font-bold text-2xl text-gray-800 leading-tight">Edit Notulen Rapat</h2>
            </div>
        </template>
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                        ...
                        <div class="p-6 border-t border-gray-100 bg-gray-50 flex justify-end gap-3">
                            ...
                            <button type="submit" :disabled="form.processing || !isQuillLoaded"
                                ...
                                {{ form.processing ? 'Menyimpan...' : 'Update Notulen' }}
                            </button>
                        </div>
                    </div>
                </form>
